...
In IP networks, SMB uses TCP protocol -- which provide 3 way handshake btw client and server before a connection is finally established.
...
IT;s an alternative implementation of the SMB server called samba, which developed for UNIX OS.
Samba implements the CIFS(Common internet File system) network protocol. 
CIFS is a specific implementation of SMB protocol originally created by Microsoft. -- this allow samba to intracte effectifely with newer windows system. therefore it often refered to as SMB/CIFS.
...
```
NiteshKS22@htb[/htb]$ smbclient -N -L //10.129.14.128
...
NiteshKS22@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445
...
```
NiteshKS22@htb[/htb]$ rpcclient -U "" 10.129.14.128
...
```
NiteshKS22@htb[/htb]$ for i in $(seq 500 1100);do rpcclient -N -U "" 10.129.14.128 -c "queryuser 0x$(printf '%x\n' $i)" | grep "User Name\|user_rid\|group_rid" && echo "";done
...
### Alternative Tools --- Impacket called samrdump.py

```
NiteshKS22@htb[/htb]$ samrdump.py 10.129.14.128
...
### SMBmap
```
NiteshKS22@htb[/htb]$ smbmap -H 10.129.14.128
...
### CrackMapExec
```
NiteshKS22@htb[/htb]$ crackmapexec smb 10.129.14.128 --shares -u '' -p ''
...
```
NiteshKS22@htb[/htb]$ git clone https://github.com/cddmp/enum4linux-ng.git
NiteshKS22@htb[/htb]$ cd enum4linux-ng
NiteshKS22@htb[/htb]$ pip3 install -r requirements.txt
```

### Enumeration

```
NiteshKS22@htb[/htb]$ ./enum4linux-ng.py 10.129.14.128 -A
...
<img width="1084" height="537" alt="image" src="https://github.com/user-attachments/assets/610c3261-2805-4cb9-94fc-6721cda7c6d7" />

**2. What is the name of the accessible share on the target?**

<img width="979" height="307" alt="image" src="https://github.com/user-attachments/assets/8ad08022-06aa-467e-9de6-3bda45bb04b1" />

**3. Connect to the discovered share and find the flag.txt file. Submit the contents as the answer.**

<img width="1124" height="779" alt="image" src="https://github.com/user-attachments/assets/a6e64247-bed6-42d1-8aae-f457e78f7a4c" />

**4.  Find out which domain the server belongs to.**
<img width="1065" height="819" alt="image" src="https://github.com/user-attachments/assets/fe132d93-7553-42bd-8b45-5992f2c23b0f" />
...
<img width="922" height="520" alt="image" src="https://github.com/user-attachments/assets/3ee811da-57df-4b40-a89b-6f53a30a9af7" />

**6. What is the full system path of that specific share? (format: "/directory/names")**

<img width="1291" height="662" alt="image" src="https://github.com/user-attachments/assets/4990f91d-870e-44dd-8f24-a91ed29e2912" />
